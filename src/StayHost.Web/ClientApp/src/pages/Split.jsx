...
export function Split() {
    ...
    return (
      <div className="shell" style={{ paddingBlock: '40px 90px' }}>
        <div className="empty-state">
          <h3>Liên kết không còn hiệu lực</h3>
          <p>Có thể lượt chia hoá đơn đã hoàn tất hoặc đã hết hạn.</p>
  ...
  const pay = async () => {
    ...
    try {
      ...
      toast('Cảm ơn bạn, phần của bạn đã được thanh toán.');
  ...
  return (
    <div className="shell shell-narrow" style={{ paddingBlock: '34px 90px' }}>
      <h1 className="section-title">Trả phần của bạn</h1>
      <p className="section-sub">
        {invite.listingTitle} · {invite.city} · mã đặt chỗ <b>{invite.reference}</b>
      </p>

      <section className="detail-section" style={{ paddingTop: 18 }}>
        <div className="kv-grid">
          <Kv label="Nhận phòng" value={longDate(invite.checkIn)} />
          <Kv label="Trả phòng" value={longDate(invite.checkOut)} />
          <Kv label="Số đêm" value={`${invite.nights} đêm`} />
          <Kv label="Khách" value={`${invite.guests} khách`} />
        </div>
      </section>
      ...
      <section className="detail-section">
        <div className="split-amount">
          ...
          <i>trong tổng {money(invite.total)} · {invite.paidCount}/{invite.peopleCount} người đã trả</i>
        </div>

        {done ? (
          <div className="book-alert"><b>Đã trả xong</b>
            <span>{invite.splitStatusLabel}. Không cần làm gì thêm.</span></div>
        ) : closed ? (
          <div className="book-alert is-error"><b>{invite.splitStatusLabel}</b>
            <span>Phần đã trả của mọi người được hoàn về phương thức ban đầu.</span></div>
        ) : <>
          <p style={{ fontSize: 13.5, color: 'var(--ink-muted)', margin: '0 0 14px' }}>
            Đơn chỉ được xác nhận khi tất cả mọi người đã trả. Hạn chót {dateTime(invite.expiresAt)}.
          </p>
          <div className="field-grid">
            <label className="form-field"><span className="cap">Tên của bạn</span>
          ...
          <button className="btn btn-primary" style={{ marginTop: 8 }} disabled={busy} onClick={pay}>
            {busy ? 'Đang xử lý…' : `Trả ${money(invite.amount)}`}
          </button>
        </>}
      </section>
    </div>
